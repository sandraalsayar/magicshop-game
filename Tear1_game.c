...
void updateTearGame1() {
	...
	for (int i = 0; i < TEARSCOUNT / 2; i++) {
		updateLeftTears1(&leftTears[i]);
	}
	for (int j = 0; j < TEARSCOUNT / 2; j++) {
		updateRightTears1(&rightTears[j]);
	}
...
void updateLeftTears1(TEARS* t) {
	...
	if (collision(t->row, t->col, t->height, t->width, bottle.row, bottle.col, bottle.height, bottle.width)  && t->active) {
		...
		updateScore1();
	// Handles not catching a tear by hiding it and removing a heart
...
void updateRightTears1(TEARS* t) {
	if (t->active) {
		if (t->row + t->height-1 <= 150) {
			t->row += t->rdel;
